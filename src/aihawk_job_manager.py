        ...
        for job in job_list:
            ...
            try:
                if job.apply_method not in {"Continue", "Applied", "Apply"}:
                    ...
                    if self.config['linkedin_connection_made'] <= self.config['linkedin_limit'] and (job.recruiter_link is not None and job.recruiter_link != ""):
                        note = self.gpt_answerer.generate_message_from_job_description(job_description=
                                                                                    f""" Job Title: {job.title}, Job Company: {job.company}, Recruiter Link: {job.recruiter_link}, Job Description: {job.description}"""
                                                                                    )
                        self.easy_applier_component.send_connection_request_with_note(job.recruiter_link, 
                                                                                    note)
                        self.config['linkedin_connection_made'] += 1
